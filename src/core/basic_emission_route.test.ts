...
Deno.test("Core preserves structured binding annotations without display text", () => {
...
`);
  ...
  if (first?.tag !== "bind") {
    throw new Error("Missing increment binding");
  }
  ...
  const core = Source.core({
    ...
    statements: [
      ...
      ...parsed.statements.slice(1),
    ],
  });

  assert_includes(Format.fmt(Core, core), "let increment: I32 -> I32 =");
});
