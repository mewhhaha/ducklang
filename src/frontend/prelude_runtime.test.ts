...
Deno.test("runtime concatenation operator appends text", async () => {
  const wat = Source.wat('@len("duck" ++ "lang")');
  const instance = await instantiate_wat(wat, "runtime_concat", {});
  const main = instance.exports.main;

  if (typeof main !== "function") {
    throw new Error("runtime concatenation test omitted main");
  }

  assert_equals(main(), 8);
});
