...
`ix/expandComptime` takes `{ textDocument: { uri }, position }` for an open
document. It returns rendered Ix source for the folded value or specialized
closure and a list of captured compile-time facts. `ix/viewStage` takes
...
`ix.viewStage`, `ix.expandComptime`, and `ix.runExample` are the corresponding
workspace commands. The run command only returns a Deno test invocation for the
client terminal; the language server does not execute it.

For Helix, bind a key to a workspace command, for example:

```toml
[keys.normal]
space-w = ":lsp-workspace-command ix.viewStage"
space-e = ":lsp-workspace-command ix.expandComptime"
space-r = ":lsp-workspace-command ix.runExample"
```
