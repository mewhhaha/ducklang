...
for (const [fixture, expected_score] of fixtures) {
  Deno.test("Codex request-permissions runs natively: " + fixture, async () => {
...
async function run_fixture(
  ...
  const instance = await instantiate_wat(wat, "request_permissions_native", {});
  ...
  try {
    ...
    throw new Error("Native request-permissions fixture trapped: " + fixture, {
      cause: error,
    });
  }
