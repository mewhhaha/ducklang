...
const fixture = "turn_profile_fixture.duck";
const fixture_url = new URL("./" + fixture, import.meta.url);

Deno.test("Codex turn profiling runs through the native backend", async () => {
  const source = TestSource.load_fragment_file(fixture_url.href);
  const wat = TestSource.wat(source);
  const instance = await instantiate_wat(wat, "turn_profile_native", {});
  ...
  try {
    ...
    throw new Error("Native turn-profile fixture trapped: " + fixture, {
      cause: error,
    });
  }
  ...
  assert_equals(score, 1_111_111, fixture);
});
