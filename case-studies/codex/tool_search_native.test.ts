...
const fixtures = [
  ["tool_search_name_fixture.duck", 1],
  ["tool_search_rank_math_fixture.duck", 1],
] as const;

for (const [fixture, expected_score] of fixtures) {
  Deno.test(
    "Codex deferred tool search runs natively: " + fixture,
    async () => {
      ...
      const instance = await instantiate_wat(wat, "tool_search_native", {});
      ...
      try {
        ...
        throw new Error(
          "Native deferred tool search fixture trapped: " + fixture,
          {
            cause: error,
          },
        );
