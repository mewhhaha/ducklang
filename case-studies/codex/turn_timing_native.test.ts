...
const fixture = "turn_timing_fixture.duck";
const fixture_url = new URL("./" + fixture, import.meta.url);

Deno.test("Codex turn timing runs through the native backend", async () => {
  const source = TestSource.load_fragment_file(fixture_url.href);
  const wat = TestSource.wat(source);
  const instance = await instantiate_wat(wat, "turn_timing_native", {});
  ...
  try {
    ...
    throw new Error("Native turn-timing fixture trapped: " + fixture, {
      cause: error,
    });
  }
