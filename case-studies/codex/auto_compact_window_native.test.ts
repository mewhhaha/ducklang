...
const fixture = "auto_compact_window_fixture.duck";
const fixture_url = new URL("./" + fixture, import.meta.url);

Deno.test("Codex auto-compaction windows run through the native backend", async () => {
  const source = TestSource.load_fragment_file(fixture_url.href);
  const wat = TestSource.wat(source);
  const instance = await instantiate_wat(
    wat,
    "auto_compact_window_native",
    {},
  );
  ...
  try {
    ...
    throw new Error("Native auto-compaction fixture trapped: " + fixture, {
      cause: error,
    });
  }
