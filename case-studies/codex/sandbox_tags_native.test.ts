...
const fixture = "sandbox_tags_fixture.duck";
const fixture_url = new URL("./" + fixture, import.meta.url);

Deno.test("Codex sandbox tags run through the native backend", async () => {
  const source = TestSource.load_fragment_file(fixture_url.href);
  const wat = TestSource.wat(source);
  const instance = await instantiate_wat(wat, "sandbox_tags_native", {});
  ...
  try {
    ...
    throw new Error("Native sandbox-tag fixture trapped: " + fixture, {
      cause: error,
    });
  }
  ...
  assert_equals(score, 11_111_111, fixture);
});
