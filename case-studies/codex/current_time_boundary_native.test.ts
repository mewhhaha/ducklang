...
const fixture_url = new URL(
  "./current_time_boundary_fixture.duck",
  import.meta.url,
);

Deno.test("Codex current-time boundaries run through the native backend", async () => {
  const source = TestSource.load_fragment_file(fixture_url.href);
  const wat = TestSource.wat(source);
  const instance = await instantiate_wat(
    wat,
    "current_time_boundary_native",
    {},
  );
